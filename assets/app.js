...
const sparks = () => '<span class="spark spark-1" aria-hidden="true"></span><span class="spark spark-2" aria-hidden="true"></span><span class="spark spark-3" aria-hidden="true"></span><span class="spark spark-4" aria-hidden="true"></span><span class="spark spark-5" aria-hidden="true"></span><span class="spark spark-6" aria-hidden="true"></span>';
const createButton = (label, action, value = '', type = 'button') => `<button type="${type}" class="create-button pressable" data-action="${action}" data-id="${escape(value)}"><span class="action-label">${escape(label)}</span><span class="button-arrow" aria-hidden="true">＋</span>${sparks()}</button>`;
const loginButton = (label, type = 'submit') => `<button type="${type}" class="login-button pressable"><span>${escape(label)}</span><span class="button-arrow" aria-hidden="true">↗</span></button>`;
const logoutButton = (label = 'Abmelden') => `<button type="button" class="logout-button" data-action="logout" aria-label="${escape(label)}"><span class="logout-sign" aria-hidden="true">↗</span><span class="logout-copy">${escape(label)}</span></button>`;
const revealButton = (label, action, value = '') => `<button type="button" class="reveal-button" data-action="${action}" data-id="${escape(value)}" data-text="${escape(label)}"><span class="reveal-base" aria-hidden="true">${escape(label)}</span><span class="reveal-fill" aria-hidden="true">${escape(label)}</span><span class="sr-only">${escape(label)}</span></button>`;
const deleteButton = (label, action, value = '', type = 'button') => `<button type="${type}" class="delete-action" data-action="${action}" data-id="${escape(value)}" aria-label="${escape(label)}"><span class="delete-symbol" aria-hidden="true">×</span><span class="delete-text">${escape(label)}</span></button>`;
...
async function animateAction(element, className = 'is-activated', milliseconds = 230) {
...
function submitButton(label, variant) {
  if (variant === 'create') return createButton(label, '', '', 'submit').replace(' data-action="" data-id=""', '');
  if (variant === 'delete') return deleteButton(label, '', '', 'submit').replace(' data-action="" data-id=""', '');
  return `<button class="button-primary pressable" type="submit"><span>${escape(label)}</span><span class="button-arrow" aria-hidden="true">↗</span></button>`;
}
function openDialog(title, content, onSave = null, submitText = 'Speichern', guarded = false, submitVariant = '') {
  const dialog = $('#editor'); focusBeforeDialog = document.activeElement; dialogSave = onSave; closeGuard = guarded;
  $('#dialog-title').textContent = title;
  $('#dialog-content').innerHTML = onSave ? `<form id="editor-form" data-submit-variant="${escape(submitVariant)}"><div class="form-grid">${content}</div><p class="form-error" role="alert" tabindex="-1" hidden></p><div class="modal-actions">${button('Abbrechen', 'close-dialog')}${submitButton(submitText, submitVariant)}</div></form>` : content;
...
function confirmation(title, copy, callback, label = 'Bestätigen') {
  openDialog(title, `<p class="span-2">${escape(copy)}</p>${check('Ich habe den Hinweis gelesen.', 'confirm')}`, async form => {
    ...
    await callback();
  }, label, false, /löschen|entfernen/i.test(label) ? 'delete' : '');
}
function recoveryDialog(code) {
  openDialog('Dein Wiederherstellungscode', `<p class="span-2">Speichere den Code außerhalb von Moduly. Er hilft bei einem vergessenen Passwort, solange die lokalen Browserdaten noch vorhanden sind. Gelöschte Browserdaten kann nur ein vorheriges JSON-Backup ersetzen. Der Code wird nur jetzt angezeigt.</p><code class="recovery-code span-2">${escape(code)}</code><div class="span-2">${button('Code als Datei sichern', 'download-recovery', code)}</div>${check('Ich habe meinen Code sicher aufbewahrt.', 'saved')}`, async form => {
...
function showAuth(mode = 'login') {
  ...
  const authSubmit = register ? createButton('Konto erstellen', '', '', 'submit').replace(' data-action="" data-id=""', '') : loginButton(recover ? 'Passwort zurücksetzen' : 'Anmelden');
  const localWarning = store.mode === 'local' ? '<div class="auth-local-warning"><strong>Wichtig: Dieses Konto existiert nur in diesem Browser.</strong><p>Wenn du Browserdaten löschst, einen privaten Tab nutzt oder das Gerät wechselst, fehlen Konto und Studienplan. Sichere nach Änderungen regelmäßig eine JSON-Datei.</p></div>' : '';
  $('#auth-content').innerHTML = `<h2>${register ? 'Dein Konto beginnt hier.' : recover ? 'Zurück in deinen Plan.' : 'Schön, dass du da bist.'}</h2><p>${register ? 'Ein eindeutiger Benutzername und ein Passwort genügen. Du bekommst einen Wiederherstellungscode.' : recover ? 'Mit deinem einmaligen Wiederherstellungscode kannst du ein neues Passwort vergeben.' : 'Melde dich an und mach dort weiter, wo du aufgehört hast.'}</p>${localWarning}<form id="auth-form" class="auth-form" data-submit-variant="${register ? 'create' : 'login'}">${field('Benutzername', 'username', '', 'text', 'required minlength="3" maxlength="24" pattern="[A-Za-z0-9_]{3,24}" autocomplete="username" autocapitalize="none" spellcheck="false"')}${recover ? field('Wiederherstellungscode', 'recoveryCode', '', 'text', 'required maxlength="100" autocomplete="off" autocapitalize="none" spellcheck="false"') : ''}${field(recover ? 'Neues Passwort' : 'Passwort', recover ? 'newPassword' : 'password', '', 'password', `required ${register || recover ? 'minlength="12"' : ''} maxlength="128" autocomplete="${register || recover ? 'new-password' : 'current-password'}"`)}${register || recover ? field('Passwort wiederholen', 'repeat', '', 'password', 'required minlength="12" maxlength="128" autocomplete="new-password"') : ''}${register && store.meta.inviteRequired ? field('Einladungscode', 'invite', '', 'text', 'required autocomplete="off"') : ''}${register ? '<p class="small">Mindestens 12 Zeichen. Der erste lokale Account wird Owner. Benutzernamen sind innerhalb dieses Browsers eindeutig.</p>' : ''}<p class="form-error" role="alert" tabindex="-1" hidden></p>${authSubmit}</form><div class="auth-alternatives">${mode !== 'login' ? revealButton('Ich habe bereits ein Konto', 'auth-mode', 'login') : `${store.meta.registration ? revealButton('Ein Konto erstellen', 'auth-mode', 'register') : '<span>Die Registrierung ist derzeit geschlossen.</span>'}${revealButton('Passwort vergessen?', 'auth-mode', 'recover')}`}</div>`;
}
async function enter() {
  state = await store.load(); $('#welcome').hidden = true; $('#app').hidden = false;
  $('#preview-notice').hidden = store.mode !== 'local';
  $('#username').textContent = store.user?.username || 'Dein Konto';
  $('#user-avatar').textContent = (store.user?.username || 'M').slice(0, 2).toUpperCase();
  $('#storage-label').textContent = store.mode === 'server' ? 'Privates Konto' : 'Lokal in diesem Browser';
  $('#admin-nav').hidden = !['admin', 'owner'].includes(store.user?.role);
  const initial = location.hash.slice(1); page = Object.hasOwn(titles, initial) && (initial !== 'admin' || ['admin', 'owner'].includes(store.user?.role)) ? initial : 'overview';
  render();
}
...
function render() {
  ...
  if (store.mode === 'local') {
    const info = store.backupInfo(), text = info.lastBackupAt ? `Letztes Backup: ${date(info.lastBackupAt * 1000, { year: 'numeric' })}${info.due ? ' · Neues Backup empfohlen.' : ''}` : 'Noch kein Backup erstellt.';
    $('#preview-notice span').innerHTML = `<strong>Nur lokal gespeichert.</strong> Werden Browserdaten gelöscht oder wechselst du Browser oder Gerät, sind deine Daten weg. ${escape(text)}`;
    $('#preview-notice').classList.toggle('is-due', info.due);
  }
  if (page === 'admin' && !adminData) loadAdmin();
}
function noProfile() {
  return `<div class="onboarding"><div><span class="chapter-number" aria-hidden="true">01</span><h2>Hier beginnt dein Studienplan.</h2><p>Lege dein Studium an oder übernimm eine geprüfte Vorlage. Danach kannst du Module, Prüfungen und Lernaufgaben eintragen. Alles lässt sich später ändern.</p><div class="inline-actions">${createButton('Studienprofil anlegen', 'profile-new')}${button('Hochschulen & Vorlagen', 'templates')}</div></div><div class="onboarding-note"><strong>Dein Plan gehört dir.</strong><p>Persönliche Anpassungen ändern keine zentralen Hochschuldaten. Fehlende Angaben bleiben als offen erkennbar.</p><span class="edited-label">Von dir bearbeitet</span></div></div>`;
}
...
function examsPage() {
  ...
  return heading('Gut vorbereitet ankommen.', 'Deine Prüfungen, Abgaben und Fristen. Jeder Termin lässt sich in deinen Kalender übernehmen.', createButton('Termin eintragen', 'event-new')) + `<div class="filter-bar">${select('Terminart', 'event-filter', { '': 'Alle Termine', ...KINDS }, filter)}${check('Vergangene Termine anzeigen', 'show-past', showPast)}</div><div class="panel events-panel">${events.length ? events.map(eventRow).join('') : empty('Keine Termine in dieser Auswahl.', 'Ergänze eine Prüfung, eine Abgabe oder eine persönliche Frist.', createButton('Termin eintragen', 'event-new'))}</div><p class="small muted">Beim Kalenderexport werden die Erinnerungen aus dem jeweiligen Termin übernommen. Nach einer Änderung muss eine bereits importierte Kalenderkopie aktualisiert werden.</p>`;
}
...
function planningPage() {
  ...
  return heading('Deine nächste Lerneinheit.', 'Plane kleine Schritte und hake ab, was du geschafft hast.', createButton('Aufgabe hinzufügen', 'task-new')) + `<div class="planning-summary"><strong>${fmt(due.reduce((n, t) => n + t.minutes, 0) / 60)} Stunden</strong><span>offener Lernaufwand bis ${date(end)} inklusive überfälliger Aufgaben</span><span>${tasks.filter(t => t.done).length} von ${tasks.length} Aufgaben erledigt</span></div><div class="task-board">${groups.map(([name, list]) => `<section class="task-column"><div class="card-heading"><h2>${name}</h2><span class="count-label">${list.length}</span></div>${list.length ? list.map(taskRow).join('') : '<p class="muted small">Hier ist gerade nichts eingetragen.</p>'}</section>`).join('')}</div>`;
}
function exportPage() {
  const local = store.mode === 'local';
  return heading('Dein Studium zum Mitnehmen.', 'Für deine Ablage, als Kalenderdatei oder zum Weiterarbeiten.') + `<div class="backup-callout ${store.backupInfo()?.due ? 'is-due' : ''}"><strong>${local ? 'Deine wichtigste Sicherung' : 'Datensicherung'}</strong><p>${local ? 'Browserdaten können ohne Vorwarnung verloren gehen. Lade die JSON-Sicherung regelmäßig herunter und bewahre sie außerhalb des Browsers auf.' : 'Bewahre deine Sicherung geschützt auf.'}</p>${primary('Jetzt JSON-Backup erstellen', 'json-export')}</div><div class="export-layout"><form id="export-form" class="panel"><h2>Studienübersicht</h2><p>Wähle aus, was in deiner Übersicht stehen soll.</p><div class="form-grid">${select('Studienprofil', 'profile', { '': 'Alle Studienprofile', ...Object.fromEntries(state.profiles.map(p => [p.id, p.name])) }, profile()?.id || '')}${select('Dateiformat', 'format', { pdf: 'PDF · Moduly Minimal', md: 'Markdown', txt: 'Textdatei', csv: 'CSV · Modulliste', ics: 'ICS · Kalender' }, 'pdf')}</div><div class="export-options">${check('Noten anzeigen', 'grades', true)}${check('Versuche anzeigen', 'attempts', true)}${check('Termine aufnehmen', 'events', true)}${check('Offene Module aufnehmen', 'open', true)}${check('Persönliche Notizen aufnehmen', 'notes')}</div><p class="small muted">CSV enthält die Modulliste, ICS die Termine. PDF wird über den Druckdialog deines Geräts erstellt.</p><button type="submit" class="button-primary pressable">Übersicht exportieren <span class="button-arrow" aria-hidden="true">↗</span></button><p class="form-error" role="alert" tabindex="-1" hidden></p></form><div class="export-aside"><article class="panel"><span class="section-label">Alles in einer Datei</span><h2>Vollständige Studienplan-Sicherung</h2><p>JSON enthält sämtliche Studienprofile, Module, Termine, Aufgaben, Einstellungen und Notizen. Passwort und Wiederherstellungscode sind ausgeschlossen.</p>${primary('JSON herunterladen', 'json-export')}<p class="small muted">Die Datei kann nach einer neuen Registrierung wieder eingelesen werden. Bewahre sie privat auf.</p></article><article class="panel"><h2>Sicherung wieder einlesen</h2><p>Du siehst den Inhalt vor dem Import. Die Studienpläne werden vollständig durch die Datei ersetzt. Das lokale Konto und sein Passwort bleiben bestehen.</p><label class="file-label">JSON-Datei auswählen<input type="file" id="import-file" accept=".json,application/json"></label></article></div></div>`;
}
function settingsPage() {
  const local = store.mode === 'local', info = store.backupInfo();
  return heading('So passt Moduly zu dir.', 'Deine Studienprofile, deine Ansicht und dein Konto.') + `<div class="settings-v1">${local ? `<section class="panel local-data-panel"><span class="section-label">Lokale Speicherung</span><h2>Deine Daten liegen nur in diesem Browser.</h2><p>Beim Löschen der Browserdaten gehen Konto, Studienpläne und lokale Meldungen verloren. Es gibt keine Synchronisierung auf andere Geräte.</p><div class="setting-row"><div><strong>Backup-Status</strong><small>${info.lastBackupAt ? `Letzte Sicherung: ${date(info.lastBackupAt * 1000, { year: 'numeric' })}` : 'Noch keine Sicherung erstellt'}</small></div>${primary('JSON-Backup', 'json-export')}</div></section>` : ''}<section class="panel"><h2>Studienprofile</h2><p>Beim Wechsel bleiben deine bisherigen Einträge erhalten. Ein Archiv gehört weiterhin nur zu deinem Konto.</p>${state.profiles.map(p => `<div class="setting-row"><div><strong>${escape(p.name)}</strong><small>${escape(p.university || 'Hochschule offen')}${p.archived ? ' · Archiviert' : ''}</small></div><div class="inline-actions">${button('Bearbeiten', 'profile-edit', p.id, 'text-button')}${button(p.archived ? 'Aktivieren' : 'Archivieren', 'profile-archive', p.id, 'text-button')}${deleteButton('Löschen', 'profile-delete', p.id)}</div></div>`).join('') || '<p class="muted">Noch kein Studienprofil angelegt.</p>'}<div class="inline-actions">${createButton('Studienprofil hinzufügen', 'profile-new')}${button('Hochschulen & Vorlagen', 'templates')}</div></section><section class="panel"><h2>Deine Ansicht</h2><p>Reihenfolge, sichtbare Bereiche, kompakte Zeilen und Bewegung selbst festlegen.</p>${button('Ansicht anpassen', 'customize')}<div class="setting-row"><div><strong>Darstellung</strong><small>Helle oder kontrastreiche dunkle Oberfläche.</small></div>${button(state.settings.theme === 'dark' ? 'Hell verwenden' : 'Dunkel verwenden', 'theme')}</div></section><section class="panel"><h2>Konto & Sicherheit</h2><div class="setting-row"><div><strong>${escape(store.user.username)}</strong><small>${local ? 'Lokales Konto in diesem Browser' : 'Privates Serverkonto'} · Rolle ${escape(store.user.role)}</small></div>${revealButton('Ändern', 'security', 'username')}</div><div class="security-actions">${revealButton('Passwort ändern', 'security', 'password')}${revealButton('Neuer Wiederherstellungscode', 'security', 'recovery')}${store.mode === 'server' ? revealButton('Andere Sitzungen abmelden', 'security', 'sessions') : ''}${logoutButton()}</div></section><section class="panel"><h2>Deine Daten</h2><p>Studiendaten und private Notizen werden ${local ? 'ausschließlich im lokalen Speicher dieses Browsers' : 'in deinem Konto'} gespeichert.</p><div class="inline-actions">${button('Export & Import öffnen', 'navigate', 'export')}${button('Frühere Version wiederherstellen', 'history')}</div><div class="danger-zone"><strong>Konto endgültig löschen</strong><p>Entfernt Studienprofile, Termine, Aufgaben und Meldungen. ${local ? 'Ohne JSON-Backup ist dies nicht rückgängig zu machen.' : ''}</p>${deleteButton('Konto löschen', 'account-delete')}</div></section><section class="panel"><h2>Fehler & Rückfragen</h2><p>${local ? 'Meldungen sind nur für Konten sichtbar, die in diesem Browser angelegt wurden. Sie werden nicht an uns oder GitHub gesendet.' : 'Deine Studiendaten werden nicht mitgeschickt.'}</p><div class="inline-actions">${button('Problem melden', 'report')}${button('Meine Meldungen', 'reports')}</div></section></div>`;
}
function adminPage() {
  ...
  const local = store.mode === 'local', roles = { user: 'Nutzer', admin: 'Admin', owner: 'Owner' };
  const users = (adminData.userList || []).map(user => `<div class="admin-user-row"><div><strong>${escape(user.username)}</strong><small>${roles[user.role] || escape(user.role)}${user.disabled ? ' · Gesperrt' : ''} · ${user.lastBackupAt ? 'Backup ' + date(user.lastBackupAt * 1000, { year: 'numeric' }) : 'Kein Backup'}</small></div><div class="inline-actions">${store.user.role === 'owner' ? select('Rolle', `admin-role-${user.id}`, roles, user.role) : ''}${user.id !== store.user.id ? button(user.disabled ? 'Entsperren' : 'Sperren', 'admin-user-toggle', user.id, 'text-button') : ''}${user.id !== store.user.id && user.role !== 'owner' ? deleteButton('Löschen', 'admin-user-delete', user.id) : ''}</div></div>`).join('');
  return heading('Verwaltung', local ? 'Verwaltung der Konten und Meldungen in diesem Browser.' : 'Meldungen bearbeiten, ohne private Studienpläne einzusehen.', button('Aktualisieren', 'admin-refresh')) + `${local ? '<div class="admin-local-scope"><strong>Lokaler Adminbereich</strong><p>Du verwaltest nur Konten, die in genau diesem Browser angelegt wurden. Konten deiner Freunde auf anderen Geräten sind technisch getrennt und hier nicht sichtbar.</p></div>' : ''}<div class="admin-status panel"><span><strong>${adminData.users}</strong> Konten</span><span>Speicher: <strong>${local ? `${Math.max(1, Math.ceil((adminData.storageBytes || 0) / 1024))} KB lokal` : escape(adminData.database)}</strong></span><span>Backup: <strong>${backup?.ok ? 'Zuletzt ' + date(backup.verifiedAt * 1000, { year: 'numeric' }) : backup ? 'Fehlgeschlagen' : 'Noch kein Backup'}</strong></span></div>${local ? `<div class="panel"><div class="card-heading"><div><span class="section-label">Lokale Konten</span><h2>Nutzer & Rollen</h2></div></div>${users || '<p>Noch keine Konten.</p>'}</div>` : ''}<div class="panel"><h2>Meldungen</h2>${adminData.reports.length ? adminData.reports.map(r => `<article class="report-entry"><span class="section-label">${escape(r.username || '')}${r.username ? ' · ' : ''}${escape(r.category)} · ${escape(r.status)}</span><p class="prewrap">${escape(r.message)}</p>${r.response ? `<p>Antwort: ${escape(r.response)}</p>` : ''}${button('Bearbeiten', 'report-review', r.id)}</article>`).join('') : '<p>Noch keine Meldungen.</p>'}</div><div class="panel"><h2>Änderungsprotokoll</h2>${adminData.audit.map(a => `<p>${escape(a.action)} · ${date(a.created_at * 1000, { year: 'numeric' })}</p>`).join('') || '<p>Noch keine Verwaltungsaktionen.</p>'}</div>`;
}
...
function editProfile(profileId) {
  ...
  openDialog(existing ? 'Studienprofil bearbeiten' : 'Dein Studium anlegen', `<p class="span-2 form-intro">Diese Angaben gehören nur zu deinem Plan. Übernimm ECTS und Voraussetzungen aus deiner Prüfungsordnung; unbekannte Angaben kannst du später ergänzen.</p>${field('Studiengang *', 'name', p.name, 'text', 'required maxlength="150" placeholder="Wie heißt dein Studiengang?"')}${field('Hochschule', 'university', p.university, 'text', 'maxlength="150" list="university-catalog" placeholder="Hochschule auswählen oder selbst eintragen"')}${field('Abschluss', 'degree', p.degree, 'text', 'maxlength="80"')}${field('Prüfungsordnung / Version', 'po', p.po, 'text', 'maxlength="80" placeholder="z. B. PO 2025"')}${field('ECTS für den Abschluss *', 'targetEcts', p.targetEcts, 'number', 'required min="1" max="1000" step="0.5"')}${field('Regelstudienzeit (Semester)', 'semesters', p.semesters, 'number', 'required min="1" max="20" step="1"')}${field('Dein aktuelles Semester', 'currentSemester', p.currentSemester, 'number', 'required min="1" max="30" step="1"')}${field('Mindest-ECTS für die Abschlussarbeit', 'thesisEcts', p.thesisEcts || '', 'number', 'min="0" max="1000" step="0.5" placeholder="Offen"')}${field('Offizielle Quelle (optional)', 'source', p.source, 'url', 'maxlength="1000" placeholder="https://…"')}<p class="small muted span-2">Erforderliche Module für die Abschlussarbeit markierst du im jeweiligen Modul. Der Voraussetzungencheck ersetzt keine Zulassungsentscheidung.</p>`, async form => {
  ...
  }, existing ? 'Änderungen speichern' : 'Studienprofil erstellen', false, existing ? '' : 'create');
}
...
function editModule(moduleId) {
  ...
  }, existing ? 'Änderungen speichern' : 'Modul erstellen', false, existing ? '' : 'create');
}
function editEvent(eventId, onDay) {
  const p = profile(); if (!p) return editProfile();
  const existing = state.events.find(e => e.id === eventId);
  const e = existing || { id: id(), profileId: p.id, moduleId: '', title: '', start: onDay ? `${onDay}T09:00` : localInput(), timezone: 'Europe/Berlin', duration: 90, kind: 'exam', location: '', notes: '', reminders: [10080, 1440] };
  const reminders = e.reminders ?? [10080, 1440];
  openDialog(existing ? 'Termin bearbeiten' : 'Ein Termin, den du im Blick hast.', `${field('Titel *', 'title', e.title, 'text', 'required maxlength="150"')}${select('Terminart', 'kind', KINDS, e.kind)}${select('Zugehöriges Modul', 'moduleId', { '': 'Kein Modul / persönliche Frist', ...Object.fromEntries(items('modules').map(m => [m.id, m.name])) }, e.moduleId)}${field('Datum und Uhrzeit *', 'start', e.start, 'datetime-local', 'required min="2000-01-01T00:00" max="2100-12-31T23:59"')}${field('Zeitzone *', 'timezone', e.timezone, 'text', 'required maxlength="60" list="timezones"')}<datalist id="timezones"><option value="Europe/Berlin"><option value="Europe/London"><option value="America/New_York"><option value="Asia/Tokyo"><option value="UTC"></datalist>${field('Dauer (Minuten) *', 'duration', e.duration, 'number', 'required min="1" max="1440" step="1"')}${field('Ort / Raum', 'location', e.location, 'text', 'maxlength="300"')}${area('Notizen', 'notes', e.notes)}<fieldset class="reminder-options span-2"><legend>Erinnerungen im Kalenderexport</legend>${check('Eine Woche vorher', 'reminders', reminders.includes(10080), '10080')}${check('Einen Tag vorher', 'reminders', reminders.includes(1440), '1440')}${check('Zwei Stunden vorher', 'reminders', reminders.includes(120), '120')}</fieldset><p class="small muted span-2">Bei doppelt vorkommender Uhrzeit während der Herbst-Zeitumstellung gilt das erste Vorkommen.${existing ? ' Aktualisiere nach dem Bearbeiten auch deine bereits importierte Kalenderkopie.' : ''}</p>`, async form => {
    const next = { ...e, title: val(form, 'title'), kind: val(form, 'kind'), moduleId: val(form, 'moduleId'), start: val(form, 'start'), timezone: val(form, 'timezone'), duration: num(form, 'duration'), location: val(form, 'location'), notes: val(form, 'notes'), reminders: new FormData(form).getAll('reminders').map(Number) };
...
function editTask(taskId) {
  ...
  }, existing ? 'Änderungen speichern' : 'Aufgabe erstellen', false, existing ? '' : 'create');
}
...
async function fullExport() {
  if (store.mode === 'server') await serverDownload(new URLSearchParams({ format: 'json' }));
    ...
    const stamp = dayKey();
    download(JSON.stringify({ format: 'moduly', version: 1, exportedAt: new Date().toISOString(), username: store.user?.username, state }, null, 2), `moduly-backup-${stamp}.json`, 'application/json');
    store.noteBackup(); render();
  }
...
async function showTemplates() {
  ...
  const templates = data.templates.map(t => { const p = t.data.profiles[0]; return `<article class="report-entry template-entry"><h3>${escape(p.name)}</h3><p>${escape(p.university)} · ${escape(p.degree)} · ${escape(p.po)} · ${t.data.modules.length} Module · ${fmt(p.targetEcts)} ECTS</p><p>${escape(t.verification || '')}</p>${t.notice ? `<p class="small muted">${escape(t.notice)}</p>` : ''}<div class="source-list">${(t.sources || []).map(source => link(source.url, source.label)).join('')}</div><div>${createButton('Persönliche Kopie erstellen', 'template-use', t.id)}</div></article>`; }).join('');
  const universities = (data.universities || []).map(university => `<div class="setting-row catalog-university"><div><strong>${escape(university.name)}</strong><small>${escape(university.location)}</small></div>${link(university.source, 'Offizielle Seite')}</div>`).join('');
  openDialog('Hochschulen & Studienvorlagen', `<p>Stand ${escape(data.checkedAt || 'offen')}. Eine Vorlage wird als persönliche Kopie übernommen. Deine Änderungen wirken nur in deinem Plan und spätere Katalogänderungen überschreiben nichts.</p><h3>Verfügbare Vorlage</h3>${templates || empty('Noch keine Vorlage verfügbar.', 'Du kannst ein eigenes Profil anlegen.', createButton('Eigenes Profil anlegen', 'profile-new'))}<h3>Erfasste Hochschulen</h3><div class="catalog-list">${universities}</div><p class="small muted">Für Hochschule Bremen ist der von dir genannte ISWI-Studiengang mit BPO 2025 erfasst. Für Jade Hochschule und Universität Bremen wurde bisher kein konkreter Studiengang mit Prüfungsordnung festgelegt.</p>`);
}
...
function reportDialog() {
  if (!store.user) return openDialog('Problem melden', '<p>Melde dich zuerst an.</p>');
  openDialog('Was funktioniert noch nicht?', `${select('Kategorie', 'category', { technical: 'Technischer Fehler', study: 'Hinweis zu Studiendaten', privacy: 'Datenschutz / Konto' })}${area('Deine Beschreibung *', 'message')}<p class="small muted span-2">Bitte keine Passwörter oder Wiederherstellungscodes angeben. Es werden keine Studienpläne, Screenshots oder Gerätedaten automatisch angehängt.</p>`, async form => {
    ...
    await store.request('reports', 'POST', { category: val(form, 'category'), message: val(form, 'message') }); toast(store.mode === 'local' ? 'Meldung lokal gespeichert' : 'Deine Meldung wurde gespeichert');
  }, 'Meldung einreichen');
}
...
function legal(kind) {
  const op = store.meta.operator || {}, local = store.mode === 'local';
  if (kind === 'imprint') {
    openDialog('Impressum', `<div class="legal-copy"><h3>Anbieter</h3>${op.name ? `<p class="prewrap">${escape(op.name)}<br>${escape(op.address)}</p><p>Kontakt: ${escape(op.email)}</p>` : '<p>Dieses private V1-Projekt enthält noch keine geschäftlichen Anbieterangaben. Vor einer öffentlichen oder geschäftlichen Nutzung müssen die Angaben des tatsächlichen Betreibers ergänzt werden.</p>'}<h3>Über Moduly</h3><p>Moduly ist ein persönlicher Studienplaner. Es besteht keine automatische Verbindung zu einer Hochschule. Deine Einträge und Berechnungen ersetzen keine offiziellen Prüfungs- oder Zulassungsentscheidungen.</p>${local ? '<p>Die GitHub-Pages-Version verarbeitet die Studienpläne im Browser und überträgt sie nicht an Moduly.</p>' : ''}</div>`); return;
  }
  openDialog('Datenschutzhinweise', `<div class="legal-copy"><p>Version 1.1 · Stand 13. September 2026</p><h3>Verantwortlich</h3><p class="prewrap">${op.name ? `${escape(op.name)}<br>${escape(op.address)}<br>${escape(op.email)}` : 'Die Betreiberangaben für eine spätere öffentliche Nutzung sind noch nicht hinterlegt.'}</p><h3>Was diese Version verarbeitet</h3>${local ? '<p>Benutzerkonten, Passwort-Hashes, Wiederherstellungscode-Hashes, Rollen, Studienprofile, Module, Noten, Versuche, Termine, Aufgaben, Meldungen und Einstellungen werden ausschließlich im lokalen Speicher dieses Browsers abgelegt. Passwörter werden nicht im Klartext gespeichert.</p><p>Es gibt keine Kontosynchronisierung und kein zentrales Moduly-Backend. Beim Löschen der Browserdaten gehen diese Daten verloren. JSON-Sicherungen werden nur auf ausdrücklichen Klick als Datei erstellt und müssen selbst geschützt aufbewahrt werden.</p><p>Der lokale Adminbereich sieht ausschließlich Konten und Meldungen desselben Browsers. Er kann keine Konten auf anderen Geräten verwalten und sendet keine Meldungen an Moduly oder GitHub.</p><p>Beim Abruf der Website kann GitHub als Hostinganbieter technisch notwendige Verbindungsdaten nach seinen Bedingungen verarbeiten.</p>' : `<p>Für dein Konto speichern wir Benutzername, Passwort-Hash, Hash des Wiederherstellungscodes, Rolle und Erstellungszeitpunkt. Hinzu kommen deine Studienprofile, Module, Noten, Termine, Aufgaben, Notizen und Einstellungen.</p><p>Die Serverinstanz verwendet ein notwendiges Sitzungscookie. Kontodaten bleiben bis zur Kontolöschung gespeichert; Sicherungen und Protokolle richten sich nach dem Betriebskonzept der Instanz.</p><h3>Hosting und Zugriff</h3><p>${escape(op.host || 'Angaben des Betreibers')}</p>`}<h3>Deine Möglichkeiten</h3><p>Du kannst deine Angaben korrigieren, exportieren, aus einer Sicherung wiederherstellen und dein Konto löschen. Die Anwendung ist kein offizielles Hochschulsystem.</p><h3>Externe Dienste</h3><p>Moduly V1 lädt keine Werbung, Analysewerkzeuge, externen Schriftarten, KI-Dienste oder Discord-Integrationen. Quellenlinks werden erst beim Anklicken geöffnet. Kalenderexporte werden als Datei heruntergeladen.</p>${op.privacyExtra ? `<h3>Weitere Angaben des Betreibers</h3><p class="prewrap">${escape(op.privacyExtra)}</p>` : ''}</div>`);
}
...
const actions = {
  ...
  'admin-user-toggle': async value => { await store.request(`admin/users/${value}`, 'POST', { action: 'toggle' }); await loadAdmin(); toast('Kontostatus geändert'); },
  'admin-user-delete': value => confirmation('Lokales Konto löschen?', 'Das Konto und alle zugehörigen Studienpläne und Meldungen werden aus diesem Browser gelöscht. Ohne eigene JSON-Sicherung ist dies endgültig.', async () => { await store.request(`admin/users/${value}`, 'DELETE'); await loadAdmin(); toast('Lokales Konto gelöscht'); }, 'Konto löschen'),
  legal,
...
document.addEventListener('click', async event => {
  ...
  if (target) {
    ...
    actionLock = true;
    try {
      if (target.classList.contains('logout-button')) await animateAction(target, 'is-leaving', 320);
      else if (target.matches('.create-button, .delete-action, .reveal-button, .login-button')) await animateAction(target);
      await action(target.dataset.id);
    } catch (error) { showError(error, $('#editor[open] .form-error')); }
...
document.addEventListener('submit', async event => {
  ...
  try {
    if (submit?.matches('.create-button, .delete-action, .login-button')) await animateAction(submit);
...
document.addEventListener('change', async event => {
  ...
  try {
    ...
    else if (input.name?.startsWith('admin-role-')) { await store.request(`admin/users/${input.name.slice('admin-role-'.length)}`, 'POST', { action: 'role', role: input.value }); await loadAdmin(); toast('Rolle geändert'); }
