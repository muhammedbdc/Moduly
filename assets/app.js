...
function showAuth(mode = 'login') {
  ...
  if (store.mode === 'preview') {
    $('#auth-content').innerHTML = `<h2>Schau dich um.</h2><p>Probiere den Studienplan direkt auf deinem Gerät aus. In dieser Vorschau bleiben deine Einträge in diesem Browser.</p><p>Für persönliche Konten auf mehreren Geräten wird Moduly auf deinem eigenen Server betrieben.</p><div class="auth-buttons">${loginButton('Vorschau öffnen', 'button').replace('<button ', '<button data-action="preview" ')}${createButton('Mit Beispielen starten', 'demo')}${revealButton('Vorschau zurücksetzen', 'reset-preview')}</div><p class="small muted">Die Beispieldaten gehören zu keiner realen Prüfungsordnung.</p>`;
    return;
  }
